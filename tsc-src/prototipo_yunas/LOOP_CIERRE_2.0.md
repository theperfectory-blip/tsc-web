...
| 7 | Quién ve plantillas | Primero "cualquier presidente"; **revertido el 18/09**: se quitó "Otros equipos" y cada presidente ve solo la suya. **Aclarado el 20/09**: la plantilla publicada SÍ va a ser pública, pero **no** en `mejoras.html` — va en la tarjeta del equipo del sitio principal, con un botón que muestra el plantel con las stats de cada jugador. Motivo: Luis comparte el save igual (como las memory cards de PES 4), así que la información no es secreta, solo está incómoda de consultar. **Pospuesto**: se hace más adelante, no en este loop. Al implementarlo hay que abrir la lectura de `pes5_plantillas` y mostrar la fecha de publicación (es una foto, no datos en vivo). Requisitos completos en `SLICE_FICHAJES_Y_TARJETA_EQUIPO.md` |
