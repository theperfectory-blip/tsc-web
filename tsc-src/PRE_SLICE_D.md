# Pre-slice — Macro Slice D · Backup/Restauración + Sorteo público en tiempo real (v3.4g — CERRADO, merge en stand-by)

> **v3.4g (2026-07-04) — Codex aprueba la corrección de `prefers-reduced-motion` en ambos modos.** Con animaciones reactivadas se reverificó que las 5 ramas normales (sorteo, fireworks, H2H/calendario, volumen, ticker/fade-rise) entran correctamente a su coreografía animada — los gates de v3.4f solo añaden una entrada temprana cuando `reduced()` es `true`, sin tocar la rama normal. Hooks de debug y monkey-patches retirados, sintaxis y `git diff --check origin/main...HEAD` limpios. **Slice D queda cerrado sin puntos pendientes de implementación.** Codex autoriza el commit y push de este cierre a `redesign/migration`, pero **el merge a `main` sigue sin autorizar** — queda en stand-by hasta nueva indicación.

> **v3.4f (2026-07-04) — corrección tras revisión de Codex.** La verificación inicial de `prefers-reduced-motion` (v3.4e) fue **incompleta**: solo cubrió los entry points genéricos de `motion.js` (`reveal`, `ticker`, `countUp`), no las coreografías propias de `sorteo.js` (7 frames + drumroll + sparks + confetti, ~5s) ni los fuegos artificiales canvas de `bracket.js` (~7s), ni varios `scrollIntoView({behavior:'smooth'})` forzados en `calendar.js`/`history.js`, ni el rebote elástico del volumen en `ui-utils.js`. Codex encontró los 4 puntos y esta ronda los corrige — ver sección 11 reescrita.
...
## 11 — `prefers-reduced-motion`: verificación inicial incompleta, corregida tras revisión de Codex (v3.4e → v3.4g, cerrado)
...
### v3.4g — regresión con animaciones reactivadas + cierre

Codex validó el fix de v3.4f y autorizó re-verificar la rama normal (no reducida) antes de cerrar. El usuario reactivó **Efectos de animación** en Windows (confirmado: `matchMedia('(prefers-reduced-motion: reduce)').matches === false`) y se repitió la misma batería, esta vez esperando que cada rama **sí** anime.

**Verificado (misma metodología: sin ejecutar sorteos reales, sin mutar Firestore):**
- **Sorteo:** con un hook de debug temporal re-expuesto (`window.__sorteoDebug`, retirado al final) y un bombo/equipo ficticios nuevos, se invocó `playDrawAnimation` con `MOTION.reduced()===false`. Con `MutationObserver` sobre `.reveal-card`/`.chibi-rig` + `isBusy()`: la reveal card pasó a `in` a los ~2057ms (esperado ~1960ms, suma de los primeros 4 pasos de `SEQ`) y volvió a `out` a los ~4477ms (esperado ~4457ms, tras el paso de confetti) — coincide con la coreografía completa de 7 frames, no la abreviada. `busy` permaneció `true` durante todo el ciclo, incluyendo los 280ms+1000ms de `finishSequence()` normal (NO la versión atómica de `finishSequenceReduced`), liberándose recién al final — confirma que la rama normal sigue usando `finishSequence()` sin alterar, tal como se diseñó.
- **Fireworks:** `launchChampionFireworks(...)` con `reduced===false` → creó `#fw-canvas` (dos corridas independientes), lanzó cohetes con audio inmediato (`_soundRocketLaunch` síncrono en el constructor de `Rocket`), y la limpieza (click → `_cleanup()`) removió el canvas correctamente en ambas corridas.
- **H2H (`history.js`):** `histH2HShow(...)` con `Element.prototype.scrollIntoView` interceptado (revertido después) → `{"behavior":"smooth","block":"center"}`.
- **Volumen (`ui-utils.js`):** arrastre simulado con `PointerEvent` reales más allá del borde → al soltar, `requestAnimationFrame` fue invocado (contador de llamadas pasó de 0 a 1, interceptado con un stub temporal) — confirma que entró a la rama de spring-back con overshoot, no al atajo `clearOvfImmediate()`.
- **Ticker:** `#ticker` pasó a tener 2 hijos (`#ticker-track` original + clon con `aria-hidden="true"`) y `transform:translateX(...)` con valor no-cero, confirmando que `MOTION.ticker()` clonó el track y arrancó el loop de desplazamiento (antes, en modo reducido, se quedaba en 1 hijo sin `transform`).
- **Fade-rise:** los nodos `[data-reveal]` recién mostrados quedaron en `opacity:0`/`data-reveal-bound="1"` sin `data-revealed` — es decir, en estado "preparado para animar", no asentados de inmediato como en modo reducido; quedan a la espera del `IntersectionObserver`.
- **Limitación de método (no bloqueante):** la pestaña automatizada estuvo en `document.visibilityState:"hidden"` durante parte de las pruebas de fireworks/volumen, lo que atenúa `requestAnimationFrame` en segundo plano (política del navegador, no del código) — impidió observar el ciclo completo de 7s/430ms en foco real. Se compensó verificando que la rama animada **se invoca** (contador de llamadas a `rAF` pasó de 0 a ≥1) en vez de medir la animación cuadro a cuadro. El código de esas ramas normales no fue tocado por el fix (solo se agregó un `return` temprano antes, cuando `reduced` es `true`), así que no hay superficie nueva de regresión más allá de lo ya confirmado.
- Sin errores de consola. Hooks de debug (`window.__sorteoDebug`) y monkey-patches (`scrollIntoView`, `requestAnimationFrame`) retirados en cada caso — `git status` sin diferencias en `sorteo.js` tras la limpieza.

**Conclusión:** `prefers-reduced-motion` funciona correctamente en ambos modos — reducido (sin coreografías/canvas/scroll suave/rebote) y normal (todas las ramas animadas intactas). **Aprobado por Codex. Cerrado.**
...
- `prefers-reduced-motion` → **corregido y cerrado (v3.4f/v3.4g, sección 11)**: Codex encontró 4 coreografías (sorteo, fuegos del bracket, scrolls forzados, rebote de volumen) que ignoraban la preferencia; corregidas en `bracket.js`, `calendar.js`, `history.js`, `sorteo.js`, `ui-utils.js`. El primer intento de fix del sorteo también fue incompleto (reutilizaba los delays de `finishSequence`) — corregido con `finishSequenceReduced()` dedicada. Reverificado en ambos modos (reducido y normal) y aprobado por Codex.
...
25. Verificación en vivo de `prefers-reduced-motion` con la preferencia del SO activada — **v3.4e incompleta, corregida en v3.4f/v3.4g** (sección 11): 4 coreografías fuera de `motion.js` (sorteo, fuegos, scrolls forzados, rebote de volumen) ignoraban la preferencia; corregidas en 5 archivos JS y reverificadas en ambos modos (reducido y normal) sin mutar datos reales — **hecho, aprobado por Codex.**
...
- **Slice D aprobado funcionalmente por Codex (2026-07-03).** Reglas Firestore de `palmares` validadas en consola el 2026-07-04 (sección 10). **`prefers-reduced-motion` cerrado (sección 11, v3.4g):** Codex encontró 4 gaps el 2026-07-04 (sorteo, fuegos, scrolls, volumen), corregidos y reverificados en ambos modos (reducido y normal, con animaciones reactivadas) — **aprobado por Codex el 2026-07-04.** Slice D cerrado sin puntos pendientes de implementación. **El merge a `main` sigue sin autorizar** — queda en stand-by por decisión explícita, no por trabajo pendiente.
...
- `prefers-reduced-motion` — **cumplido (v3.4f/v3.4g, sección 11):** 4 gaps encontrados por Codex corregidos (sorteo, fuegos, scrolls, volumen); reverificado en ambos modos (reducido y normal) y aprobado por Codex.
